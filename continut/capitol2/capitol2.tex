...
În acest capitol se evidențiază istoria dispozitivelor VR, cât și detalii despre tehnologiile de ultimă generație și modul în care au fost folosite.
...
Deși nu era un HMD și nu necesita un calculator extern în sensul modern, Sensorama a reprezentat un precursor important al experiențelor de tip "immersive", demonstrând potențialul stimulării multiplelor simțuri. 
Modul de folosire a acestora este exemplificat în figura 2.1.
...
\end{wrapfigure}
Trecerea de la experimente academice la produse orientate către consumatori a fost marcată decisiv de lansarea primului dispozitiv VR fabricat în masă și disponibil publicului larg: Oculus Rift DK1 (fig 2.2), în martie 2013, la un preț de aproximativ 300 de dolari (aproximativ 450\$ în anul 2025, după calcularea inflației). Apariția sa a fost strâns legată de progresele în tehnologia ecranelor compacte de înaltă rezoluție (impulsionate de industria telefoanelor mobile) și, crucial, de creșterea exponențială a puterii de calcul și accesibilității computerelor personale (PC-urilor). 
...
Fiind succesorul lui Unreal Engine 4, Unreal Engine 5 (UE5)\cite{unreal:learning} este un motor de joc („game engine”) care permite realizarea unor experiențe interactive de ultimă generație, fiind mult mai ușor de folosit decât dezvoltarea unui motor grafic de la zero\cite{unreal:homepage}. Punctul de plecare este același pentru toți dezvoltatorii, însă UE5 oferă o infinitate de direcții și domenii spre care aceștia se pot orienta. În majoritatea cazurilor sunt create jocuri, dar există și numeroase exemple extra-ludice în care motorul este extrem de util.
...
Noutățile din 5.4 \cite{unreal:docs} (folosită în proiectul VR Museum):
...
\end{itemize}
In figura 2.4 se poate observa un exemplu de funcționalitate a Ray Tracing-ului. Această tehnologie nu este nouă; ea este folosită de mult timp în cinematografie, mai ales în filmele de animație 3D produse de studiouri precum Pixar, DreamWorks sau Disney. În producția de filme, fiecare cadru poate fi randat în minute ori chiar ore pe multe servere, unde costul nu este constrâns de cerințe de timp real. Din acest motiv, tehnica nu era luată în considerare pentru jocurile video: hardware-ul existent putea randa doar 2–5 cadre pe secundă în condiții optime, mult sub pragul necesar pentru un gameplay fluent. Un exemplu de comparație se regăsește in fig 2.5.
\begin{figure}[htbp]
  \centering
  \includegraphics[width=8cm]{continut/capitol2/figuri/ray_tracing.jpg}
  \caption{Ray-tracing basics\protect\footnotemark}
  \label{fig:ray-tracing}
\end{figure}
\footnotetext{Sursă: NVIDIA Developer — \emph{Ray Tracing Fundamentals}.}
...
\begin{figure}[ht]
  \centering
  \includegraphics[width=.8\linewidth]{continut/capitol2/figuri/RTX_ON_OFF.png}
  \caption[Cyberpunk 2077 – RT ON/OFF]{Cyberpunk 2077 – comparaţie Ray Tracing \textsc{ON} (stânga) vs.\ \textsc{OFF} (dreapta)\protect\footnotemark.\ 
  Diferenţa este evidentă la nivelul reflexiei apei, mult mai pronunţată şi detaliată în modul \textsc{ON}.}
  \label{fig:RayTracing}
\end{figure}

\footnotetext{Captură de ecran preluată din videoclipul YouTube: \url{https://www.youtube.com/watch?v=F4HkXEzzcRw}.}



\section{Evoluția hardware}

Acest lucru s-a schimbat în anul 2018, când Nvidia a lansat prima serie de plăci video ce pot simula lumina în timp real, seria RTX 20. Aceste plăci video aveau câteva îmbunătățiri față de seria precedentă, GTX 16: Noi nuclee RT făcute special pentru Ray Tracing, nuclee Tensor pentru DLSS, mult mai multe nuclee CUDA, memorie mai rapidă și shadere Mesh, folosite pentru a susține geometrie mult mai avansată și complexă fără a pierde din perfomanță.
...
Prescurtat DLSS\cite{nvidia:dlss} și având 4 versiuni în momentul actual, acest algoritm folosește nucleele Tensor pentru a spori calitatea imaginii. Jocul este randat la o rezoluție mai mică, iar placa video folosește tehnologia DLSS pentru a face “upscaling” la imagine, astfel permițându-ne să jucăm la setari grafice mult mai mari și desigur, cu Ray Tracing pornit. Versiunile de DLSS, anul lansării și noutățile aduse sunt prezentate in tabelul 2.1, iar in tabelul 2.2 se pot observa modurile actuale.
...
\begin{table}[h]
  ...
  \begin{tabular}{||c c c||}
    ...
    \hline
  \end{tabular}
    \caption{Generații de DLSS}
  \label{tab:dlss-generations}
\end{table}
...
Prima evoluție s-a intamplat în anul 2020 când Nvidia a lansat seria de plăci video RTX 30, unde și-a făcut apariția DLSS 2. Acesta nu numai ca este mult mai eficient și antrenat pe un model mai performant, ci are 4 noi setări.
...
\begin{table}[htbp]
  ...
  \begin{tabular}{||l c||}
    ...
    Quality           & 66,6\,\% \\[1ex]
    \hline
    Native Upscale    & 100\,\% (DLAA, introdus în DLSS 2.2) \\[1ex]
    \hline
  \end{tabular}

  \caption{Modurile DLSS}
  \label{tab:dlss-res}
\end{table}
...
Dacă jocul este randat la 1080p (1920×1080), rezoluția reală înainte de acest “upscale” realizat de DLSS este prezentată in tabelul 2.3.
...
\begin{table}[htbp]
  ...
  \begin{tabular}{||c c||}
    ...
    \hline
    Native Upscale   & 1920×1080 \\[1ex]
    \hline
  \end{tabular}
  \caption{Rezoluții interne pentru modurile DLSS}
  \label{tab:dlss-res}
\end{table}
...
Apărut odată cu Unreal Engine 5 și cu Nanite, tehnologia Lumen simuleaza lumina în timp real prin folosirea unor tehnici foarte optimizate.
...
\noindent Lumen poate fi folosit și cu Ray Tracing la nivel hardware, pentru un nivel de detaliu mai avansat fără a afecta performanța atat de mult față de Ray Tracing standard.
...
\begin{adjustwidth}{2em}{0pt}
\begin{description}
    \item[\textbf{Experiențe 360°}] - bazate pe fotografii sau videoclipuri sferice, unde utilizatorul se poate uita în jur, dar nu poate interacționa sau să se deplaseze;    
